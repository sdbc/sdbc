...
{
	return hash_func(fast_read(src, 3));
}
...
ShowLog(1,"%s:at 1,src=%d,dst=%d",__FUNCTION__,
	(int)(src-source),(int)(dst- destination));
				return 0;
			}
...
ShowLog(1,"%s:at 2,src=%d,dst=%d",__FUNCTION__,
	(int)(src-source),(int)(dst- destination));
				return 0;
			}
#endif
...
ShowLog(1,"%s:at 3,src=%d,dst=%d",__FUNCTION__,
	(int)(src-source),(int)(dst- destination));
				return 0;
			}

			if(matchlen > (ui32)(last_destination_byte - dst - UNCOMPRESSED_END + 1)) {
ShowLog(1,"%s:at 4,src=%d,dst=%d",__FUNCTION__,
	(int)(src-source),(int)(dst- destination));
				return 0;
			}
#endif
...
#ifdef QLZ_MEMORY_SAFE
					if(src > last_source_byte) {
ShowLog(1,"%s:at 5,src=%d,dst=%d,cword_val=%d",__FUNCTION__,
	(int)(src-source),(int)(dst- destination),cword_val);
						return 0;
					}
#endif
					*dst = *src;
...
size_t qlz_compress(const void *source, char *destination, size_t size)
{
	...
	ui32 compressed;
	size_t base;
	qlz_state_compress q_state,*state=&q_state;
...
{
qlz_state_decompress q_state,*state=&q_state;
	state->stream_counter = 0;
...
if(!dsiz) ShowLog(1,"%s:at 1 dsiz=0!",__FUNCTION__);
		} else {
			if(csiz != dsiz + qlz_size_header(source)) {
ShowLog(1,"%s:at 1,csiz=%d,dsiz=%d,qlz_size_header=%d",__FUNCTION__,
	csiz,dsiz,qlz_size_header(source));
				return 0;
			}
	...
	unsigned char *dst = state->stream_buffer + state->stream_counter;
		if((*source & 1) == 1) {
			...
			if(csiz != dsiz + qlz_size_header(source)) {
ShowLog(1,"%s:at 2,csiz=%d,dsiz=%d,qlz_size_header=%d",__FUNCTION__,
	csiz,dsiz,qlz_size_header(source));
				return 0;
			}
...
#endif
if(!dsiz) ShowLog(1,"%s:final dsiz=0!",__FUNCTION__);
	return dsiz;
}
