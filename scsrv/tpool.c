...
int TCB_get(TCB **rp)
{
	...
	task=(*rp)->next;//找到队头
	if(!task->next) {
		ShowLog(1,"%s:TCB=%d,错，未在队列中！",__FUNCTION__,task->sv.TCB_no);
		return -2;
	}
	if(task->next == task) *rp=NULL;//最后一个了
	else (*rp)->next=task->next;//新的队头
	task->next=NULL;
...
//协商密钥
			...
			if(task->head.PROTO_NUM==65535) {
				...
				if(!task->head.PROTO_NUM) {
					...
					ShowLog(1,"%s:TCB:%d,未登录",__FUNCTION__,task->sv.TCB_no);
					ret=-1;
					break;
				}
...
int check_TCB_timeout()
{
	...
	for(i=0;i<cltnum;i++,task++) {
		...
		if(task->next) {
			ShowLog(2,"%s:TCB_no=%d,已经在队列中",__FUNCTION__,i);
			continue;
		}
		if(task->call_back) {
			char buf[30];
