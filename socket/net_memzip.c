...
//释放接收buf
	...
	i=connect->SendLen-len;
        if(i<(100+HEADPACKLENGTH)||i>SDBC_BLKSZ) {
			...
			connect->SendBuffer=malloc((size_t)connect->SendLen+400);
	...
	if(nethead->PKG_LEN){
		...
		if(connect->CryptFlg & DO_ZIP && !(connect->CryptFlg & UNDO_ZIP)) {
			nethead->T_LEN=memzip(p,nethead->data,nethead->PKG_LEN);
/*
...
//free SendBuffer
	...
	if(zbuf != connect->RecvBuffer) {
		i=memunzip(zbuf,nethead->T_LEN,connect->RecvBuffer,nethead->PKG_LEN);
		free(zbuf);
